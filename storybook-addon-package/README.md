...
- **Vue 3.5+** — the component gives a prop its default value inside the `defineProps` destructure. Vue only keeps a destructured prop reactive from 3.5, so on an earlier version the default is dropped and the component stops updating when a parent changes that prop.
- **Svelte 5** — `$props()`, `$state()` and the `Snippet` type.
- **Angular 17.1+** — `input()` signals.

React and Solid add no floor beyond what the addon itself needs. None of this affects the dependency graph or anything else `sb-deps` does — it only describes the starter code it writes, so on an older version replace the generated body with whatever your project uses. Or replace the template outright via [`scaffold`](#scaffold), which is what that option is for.
