...
Creating the empty story file beside a component is how you ask for a story for one `sb-deps` would otherwise leave alone: `helpers.stories.ts` finds `helpers.ts` whatever is in it. A dotted name is the exception, and `sb-deps` says nothing about it — with no marker `Button.test.stories.ts` writes nothing, since `Button.test.ts` is not a component here, and with a marker it writes a new `Button.test.lit.ts` rather than finding your `Button.test.ts`. Name a component without dots if you want a story for it.
