...
`check` runs the type check and nothing else. It reports without writing anything, so it is the one to run in an automated setting; `lint` and `format` stay separate.
