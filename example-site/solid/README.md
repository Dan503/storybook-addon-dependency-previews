...
There is no plain Node build to run, so nothing here serves the built site the way Netlify does — use `npm run dev` for local work. To build for somewhere other than Netlify, change that preset in `vite.config.ts`.
